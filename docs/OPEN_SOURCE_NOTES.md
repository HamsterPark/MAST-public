...
这是项目已有的长时间仪器控制经历，涉及当时的外部控制路径。该经历由维护者提供；实验数据和完整运行日志不随本快照发布。

MAST 6.5.0 在已有实践基础上新增 `/api/ext/v1` 及配套 MCP 集成。新接口已有软件测试，尚待真机验证。
项目的真机实践与公开版的软件验证构成两类记录：五天五夜对应上一版本的实际部署，
...
- **未发表研究与现场数据保护**：样品研究、实验设计与记录、仪器标定、站点配置和凭据不随仓提供。
...
**未发表的样品研究**也不在本仓：某一样品 campaign 的实验设计及其 conduct 模板、实测数据与参数、
以及 prompt 示例里引用的真实研究假设（改成了不指向具体样品的写法）。
...
通用技术推理保留；本站的实验记录、样品测量值与仪器标定值不作为公开叙事或测试夹具。
