"""Paper Writing agent — Phase 6 real implementation.
...
`model` defaults to make_chat_model("paper_writing") →
AGENT_MODEL["paper_writing"], which per operator request (2026-07-18) is
Kimi K3, NOT a Claude model. Override per call
with model_id= if you want Anthropic. Pass a GenericFakeChatModel in tests.

Paper Writing does NOT include SafetyGateMiddleware or HumanInTheLoopMiddleware
— it is a read-only document-assembly agent that never commands the instrument.
"""
    ...
    *,
    ...
    Args:
        ...
        model:           A chat model instance OR a fake chat model for tests.
                         Default: make_chat_model("paper_writing") →
                         AGENT_MODEL["paper_writing"] (Kimi K3 per operator
                         request; not a Claude model).
    ...
    if model is None:
        model = make_chat_model(
            ...
            temperature=0.3,  # creative writing but anchored to facts
        )
    ...
    middleware = [
        *(extra_middleware or []),
        ...
        UpstreamArtifactMiddleware("paper_writing"),
        ...
        StallGuardMiddleware(agent_name="paper_writing"),
        *make_call_limit_middleware(
    ...
    if turn_recorder is not None:
        ...
        middleware.append(RecorderMiddleware(agent_id="paper_writing", recorder=turn_recorder))

    _system_prompt = resolve_prompt("agent.paper_writing.system", SYSTEM_PROMPT)
    ...
    record_build("paper_writing", middleware=middleware, tools=tools,
                 system_blocks=[("agent.paper_writing.system", _system_prompt)],
                 standalone=standalone)
    logger.info(
        "paper_writing agent built: %d tools, %d middlewares",
        len(tools),
        len(middleware),
    )
    return agent
