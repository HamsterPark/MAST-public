...
**在 MAST 6.4.0 的一轮实机实验中，外部 AI agent 控制真实 STM，带时间戳的记录覆盖约 93 小时。**
...
- **MAST 6.4.0 真机实践**：外部 AI agent 控制真实 STM，带时间戳的记录覆盖约 93 小时。
...
- **新增外部接口**：`/api/ext/v1` 与配套 MCP 集成将项目已有的外部控制能力扩展为公开接口。
  新路径已经过软件测试，真机验证尚待完成；上述 93 小时记录使用 MAST 6.4.0 的控制路径。
